...
class TestEvaluateScript(unittest.TestCase):
    ...
    def test_onsets(self):
        ...
        sum_res = np.fromiter(res[1].split(',')[1:], dtype=np.float64)
        self.assertTrue(np.allclose(
            sum_res, [14, 2, 0, 1, 15, 0.875, 0.933, 0.903, 0.824]))
        # third line contains the mean results
        mean_res = np.fromiter(res[2].split(',')[1:], dtype=np.float64)
    ...
    def test_beats(self):
        ...
        res = np.fromiter(res[1].split(',')[1:], dtype=np.float64)
    ...
    def test_chords(self):
        ...
        weighted = np.fromiter(res[1].split(',')[1:], dtype=np.float64)
        ...
        piecewise = np.fromiter(res[2].split(',')[1:], dtype=np.float64)
